...
with col1:
    ...
    if sum(aArea+bArea+cArea+dArea) == 100:
        ...
        Ia2 = min(0.2*S2,P)
        Fa2 = S2*(P-Ia2)/(P-Ia2+S2)
        
        CN1 = (4.2*CN2)/(10 - (0.058*CN2))
        S1 = 1000/CN1 - 10
        Pe1 = (P - 0.2*S1)**2/(P + 0.8*S1)
        Ia1 = min(0.2*S1,P)
        Fa1 = S1*(P-Ia1)/(P-Ia1+S1)        
        
        CN3 = (23*CN2)/(10 + (0.13*CN2))
        S3 = 1000/CN3 - 10
        Pe3 = (P - 0.2*S3)**2/(P + 0.8*S3)
        Ia3 = min(0.2*S3,P)
        Fa3 = S3*(P-Ia3)/(P-Ia3+S3)        
        
        
        st.write(pd.DataFrame({'CN(I)':[CN1,S1,Pe1,Ia1,Fa1],
                               'CN(II)':[CN2,S2,Pe2,Ia2,Fa2],
                               'CN(III)':[CN3,S3,Pe3,Ia3,Fa3]},
                              index=["CN","S [in]","Pe [in]","Ia [in]","Fa [in]"]))
    else:
        flag = True
